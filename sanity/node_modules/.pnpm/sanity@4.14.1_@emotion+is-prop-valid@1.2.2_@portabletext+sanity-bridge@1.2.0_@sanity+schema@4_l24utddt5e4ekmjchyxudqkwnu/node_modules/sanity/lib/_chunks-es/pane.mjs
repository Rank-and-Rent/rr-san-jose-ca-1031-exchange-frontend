...
const TooltipContent$1 = (t0) => {
  ...
  } = useSingleDocRelease(), handlePerspectiveChange = useCallback((perspective) => () => {
  ...
  }, [setPerspective, setParams, params, defaultPerspective, onSetScheduledDraftPerspective]), schemaType = schema.get(documentType), isLiveEdit = schemaType ? isLiveEditEnabled(schemaType) : !1, isPublishedChipDisabled = useMemo(() => isLiveEdit && !selectedReleaseId ? !1 : !editState?.published, [isLiveEdit, selectedReleaseId, editState?.published]), getReleaseChipState = useCallback((release) => {
    if (!params?.historyVersion) {
