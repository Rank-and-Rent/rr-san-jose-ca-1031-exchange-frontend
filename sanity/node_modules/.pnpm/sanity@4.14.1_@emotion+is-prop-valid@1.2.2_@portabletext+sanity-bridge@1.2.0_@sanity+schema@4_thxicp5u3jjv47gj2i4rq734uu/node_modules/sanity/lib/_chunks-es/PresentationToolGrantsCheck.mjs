...
function useParams({
  ...
  }), [params.changesInspectorTab, params.comment, params.inspect, params.instruction, params.path, params.pathKey, params.parentRefPath, params.rev, params.since, params.template, params.templateParams, params.view, params.scheduledDraft]), searchParams = useMemo(() => pruneObject({
...
function PresentationTool(props) {
  const {
    ...
    state: routerState
  } = useRouter(), routerSearchParams = useUnique(Object.fromEntries(routerState._searchParams || [])), canSharePreviewAccess = useSelector(previewUrlRef, (state) => state.context.previewMode?.shareAccess !== !1), [devMode] = useState(() => {
