...
async function checkRequiredDependencies(context) {
  ...
  const {
    workDir: studioPath,
    output
  } = context, [studioPackageManifest, installedStyledComponentsVersion, installedSanityVersion] = await Promise.all([await warnAboutMissingAppId.readPackageManifest(path__default.default.join(studioPath, "package.json"), defaultStudioManifestProps), await warnAboutMissingAppId.readModuleVersion(studioPath, "styled-components"), await warnAboutMissingAppId.readModuleVersion(studioPath, "sanity")]), wantedStyledComponentsVersionRange = _package.peerDependencies["styled-components"];
