...
const TooltipContent$1 = (t0) => {
  ...
  const {
    ...
    documentId
  }), workspace = sanity.useWorkspace(), {
    onSetScheduledDraftPerspective
  } = sanity.useSingleDocRelease(), handlePerspectiveChange = React.useCallback((perspective) => () => {
  ...
  }, [setPerspective, setParams, params, defaultPerspective, onSetScheduledDraftPerspective]), schemaType = schema.get(documentType), isLiveEdit = schemaType ? isLiveEditEnabled(schemaType) : !1, isPublishedChipDisabled = React.useMemo(() => isLiveEdit && !selectedReleaseId ? !1 : !editState?.published, [isLiveEdit, selectedReleaseId, editState?.published]), getReleaseChipState = React.useCallback((release) => {
    if (!params?.historyVersion) {
